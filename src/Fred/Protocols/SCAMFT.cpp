...
bool SCAMFT::SCApad(string& line)
{
    if (line == "svl_connect" || line == "master" || line == "slave")
    {
        return true;
    }
    ...
    if (scaParts.size() != 2)
    {
        throw runtime_error("SCA MFT comma is missing or more than one comman occured!");
    }
    ...
    if (scaParts[1] == "wait") //SCA wait
    {
        ...
        {
            throw runtime_error("SCA MFT invalid wait value");
        }
    ...
    {
        throw runtime_error("SCA MFT 32 bits exceeded!");
    }
...
vector<string> SCAMFT::generateMessage(Instructions::Instruction& instructions, vector<string>& outputPattern, vector<string>& pollPattern, ProcessMessage* processMessage)
{
    string message;
...
void SCAMFT::checkIntegrity(const string& request, const string& response)
{
    vector<string> respCommands({ "svl_connect", "master", "slave" });
    string responseTemp = response;

    for (size_t i = 0; i < respCommands.size(); i++)
    {
        while (responseTemp.find(respCommands[i]) != string::npos)
        {
            responseTemp.erase(responseTemp.find(respCommands[i]), respCommands[i].length());
        }
    }
    ...
    for (size_t i = 0; i < responseTemp.size(); i++)
    {
        ...
        {
            throw runtime_error("SCA MFT: Invalid character received in RPC data:\n" + response + "\n");
        }
    }
    ...
    {
        throw runtime_error("SCA MFT: Invalid number of lines received!");
    }
    ...
    for (size_t i = 0; i < resVec.size(); i++)
    {
        ...
        bool respCommandFound = false;
        for (size_t i = 0; i < respCommands.size(); i++)
        {
            if (reqVec[i] == respCommands[i] && resVec[i] == reqVec[i])
            {
                respCommandFound = true;
                break;
            }
        }
        ...
        throw runtime_error("SCA MFT: Integrity check of received message failed!");
    }
}
 ...
 */
vector<vector<unsigned long> > SCAMFT::readbackValues(const string& message, const vector<string>& outputPattern, Instructions::Instruction& instructions)
{
...
uint32_t SCAMFT::getReturnWidth()
{
    return SCAMFT_RETURN_WIDTH;
}
